...
class DeepgramFluxTTSSettings(TTSSettings):
    ...
    The Flux voice and language are carried by ``model`` as a single
    ``flux-{voice}-{language}`` identifier (e.g. ``flux-alexis-en``).
    """
...
class DeepgramFluxTTSService(InterruptibleTTSService):
    ...
    Example::
        ...
        tts = DeepgramFluxTTSService(
            ...
            settings=DeepgramFluxTTSService.Settings(model="flux-alexis-en"),
        )
    """
